...
import {ACMEChallenges} from "./ACME"
    ...
    req.on("end", async()=>{
        try {
            ...
            if (Data.method == 'lookup' && Data.parameters.qtype == 'SOA') {
                ...
                if (Domain.length)
                    Result.push({"qtype":"SOA", "qname":Data.parameters.qname, "content":"notdns.win. hostmaster.notdns.win. " + Math.floor((new Date()).getTime()/1000) + " 30 30 30 5", "ttl": 5})
                ...
                if (ACMEChallenges[Hostname] && Data.parameters.qtype != 'NS') {
                    Result.push({"qtype":"TXT", "qname":Data.parameters.qname, "content":ACMEChallenges[Hostname], "ttl": 5})
                ...
                if (Domain.length) {
                    Result.push({"qtype":"SOA", "qname":DomainName + ".", "content":"notdns.win. hostmaster.notdns.win. " + Math.floor((new Date()).getTime()/1000) + " 30 30 30 5", "ttl": 5})
